...
void state1_behavior(int i){	//èâä˙èÛë‘
	//robot[0].dx = -1;
	//robot[0].dy = 0;
...
void state2_behavior(int i){	//íPëÃâ^î¿
	//robot[0].dx = 0.1;
	//robot[0].dy = -1;
	//robot[0].ang_r = atan2(robot[0].dy, robot[0].dx);
	...
	if(p_density != 0)
		add_pheromone(i);	//ÉtÉFÉçÉÇÉììYâ¡
}


void state3_behavior(int i){	//óUà¯ÅEâùòH
	//robot[0].dx = -1;
	//robot[0].dy = 0;
	//robot[0].ang_r = atan2(robot[0].dy, robot[0].dx);
